...
currentAccount: any;
    cashBookBalances: CashBookBalance[];
    error: any;
    success: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.cashBookBalanceService.search({
        ...
        this.cashBookBalanceService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/cash-book-balance'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: CashBookBalance) {
        return item.id;
    }
